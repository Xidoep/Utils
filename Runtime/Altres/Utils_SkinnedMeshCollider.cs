...
public class Utils_SkinnedMeshCollider : MonoBehaviour
{
    // Public variables
    public bool forceUpdate;

    // Instance variables
    public CWeightList[] nodeWeights;    // array of node weights (one per node)
    public Vector3[] newVert;        // array for the regular update of the collision mesh

    private Mesh mesh;       // the dynamically-updated collision mesh
    public MeshCollider collide;    // quick pointer to the mesh collider that we're updating
    ...
    // object.
    [ContextMenu("UpdateCollisionMesh")]
